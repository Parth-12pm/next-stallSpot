// src/components/applications/ConfirmationDialog.tsx
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function ConfirmationDialog({ open, onOpenChange, onConfirm, isSubmitting, data }: ConfirmationDialogProps) {
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="max-w-2xl">
        <AlertDialogHeader>
          <AlertDialogTitle>Confirm Application Submission</AlertDialogTitle>
          <AlertDialogDescription>Please review your application details before submitting.</AlertDialogDescription>
        </AlertDialogHeader>
              ...
              <div className="space-y-3">
                {data.products.map((product, index) => (
                  <Card key={index} className="p-4">
                    ...
                    <p className="text-muted-foreground text-sm mt-1">{product.productDetails}</p>
                  </Card>
                ))}
              </div>
            </div>
