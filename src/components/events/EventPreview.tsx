...
interface EventPreviewProps {
  eventId: string;  // Changed from id to eventId to match usage
}

export default function EventDetails({ eventId }: EventPreviewProps) {
    ...
    <Badge variant="outline">₹{eventData.entryFee} Entry Fee</Badge>
              ...
              <div className="grid gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {new Date(eventData.startDate).toLocaleDateString('en-US')} - {new Date(eventData.endDate).toLocaleDateString('en-US')}
                  </span>
                </div>
                <div className="flex items-center gap-2">
